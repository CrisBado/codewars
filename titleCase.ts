...
const titleCase = (title: string, minorWords?: string): string => {
  if (!title) return title;

  const cap = (word: string): string =>
    word.charAt(0).toUpperCase() + word.slice(1);

  const minors: string[] = (minorWords || "").toLowerCase().split(" ");

  const result: string = title
    .toLowerCase()
    .replace(/\S+/g, (word: string) =>
      minors.includes(word) ? word : cap(word),
    );
